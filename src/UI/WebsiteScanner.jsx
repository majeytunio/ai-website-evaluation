...
import React, { useState, useEffect } from 'react';
import {
  Container, Modal, Alert, Form, Button,
  ProgressBar, Card, ListGroup, Row, Col, Spinner
...
const WebsiteScanner = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserPlan = async () => {
      ...
      if (data) {
        setUserPlan(data.plan); // starter, pro, business
      }
    };
    fetchUserPlan();
  }, [navigate]);
  ...
  const handleScan = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/scan_website`,
  ...
  //   if (!website || !company || !description) {
  //     setErrorTitle("Missing Information");
  //     setErrorMessage("Please fill in all fields before exporting PDF.");
  //     setShowErrorModal(true);
  //     return;
  //   }
  ...
  //   try {
  //     const response = await axios.post(
  //       `${process.env.REACT_APP_API_URL}/scan_website/pdf`,
  //       { website, company, description },
  //       {
  //         headers: { Authorization: `Bearer ${token}` },
  //         responseType: 'blob'
  ...
  //     link.href = url;
  //     link.setAttribute('download', 'ai_readiness_report.pdf');
                  ...
                  <div className="row row-cols-1 row-cols-md-2 g-4">
                    ...
                    <div className='p-2'>
                      {/* <Button
                        ...
                        disabled={userPlan === 'starter'} // <-- disable for starter
                        title={userPlan === 'starter' ? "Upgrade to Pro for PDF export" : ""}
                      >
                        Export PDF
                      </Button> */}
                      ...
                      <Button
                        className="btn-secondary w-100"
                        disabled={pdfLoading} // prevents multiple clicks
                        onClick={handleDownloadPDF}
                      >
                        {pdfLoading ? (
                          <>
